...
export default function Research() {
  ...
  const tabs = [
    { id: "publications", label: "Journal Publications", count: publications.length },
    { id: "conferences", label: "Conference Proceedings", count: conferences.length },
    { id: "workshops", label: "Workshops Organised", count: workshopsOrganised.length },
  ];
  ...
  return (
    <div className="w-full flex flex-col items-center">
      {/* UW-Style Header (Math/Blackboard abstract image) */}
      <PageHeader 
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`relative px-8 py-4 text-sm font-bold uppercase tracking-widest transition-colors duration-200 flex items-center gap-3
                  ${isActive ? "text-iitp-blue bg-slate-50" : "text-slate-500 hover:text-slate-900 hover:bg-slate-50"}
                `}
              >
                {tab.label}
                ...
                {isActive && (
                  <motion.div layoutId="activeTab" className="absolute bottom-[-2px] left-0 w-full h-[3px] bg-iitp-blue" />
                )}
              </button>
            );
          })}
                ...
                <motion.div variants={containerVars} initial="hidden" animate="show" className="space-y-0">
                  {publications.map((pub, idx) => (
                    <motion.div key={idx} variants={itemVars} className="flex gap-6 py-6 border-b border-slate-100 hover:bg-slate-50 transition-colors px-4 group">
                      <span className="text-slate-400 font-serif text-xl group-hover:text-iitp-blue transition-colors">
